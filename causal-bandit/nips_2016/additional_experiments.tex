...
In figure \ref{fig:simple_vs_T_misspecified} we additionally show the performance of Algorithm 1. We exclude actions on $Z$ from the set of allowable actions to demonstrate how Algorithm 1 can fail in the presence of a confounding variable, as it incorrectly assumes $P(Y|do(X)) = P(Y|X)$ leading to biased estimates. We let $P(Z) = .6$, $P(Y|\boldsymbol{X}) = X_7 \oplus X_N$ and $P(X|Z)$ be given by:
