...
# These are the experiments as run in the paper but with 1000, rather than 10,000 simulations.
# The total time to run them with these settings on a modern laptop is ~ 1 hour.
...
plot_params = (regret,m_vals,T)
with open(filename,"wb") as out:
    pickle.dump(plot_params,out)   
plot_regret_versus_m(*plot_params)
...
with open(filename,"wb") as out:
    pickle.dump(plot_params,out) 
plot_regret_versus_T(*plot_params)
...
#### Data can also be plotted directly from a pickle file like below
#with open(filename,"rb") as f:
#    plot_regret_versus_T(*pickle.load(f))
