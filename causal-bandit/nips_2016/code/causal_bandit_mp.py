...
def logfile():
    return str(int(time()))+"_"+"".join(np.random.choice(list(string.lowercase),size=5))+".log"
...
def plot_regret_vs_T(regret,T_vals,a):
...
def run_parallel_simulations(experiment, simulations, processes, parameters, key_parameters):
    """ assumes the experiment function takes a keyword arguament named 'simulations' and returns a numpy array with the simulations being the 3rd dimension"""
    ...
    print "TASKS",tasks
    results = p.map_async(experiment,tasks)
    merged = np.concatenate([x for x in results.get()],axis=2)
    return merged
    
def wrapper(tpl):
    parameters,key_parameters = tpl
    return regret_vs_T(*parameters,**key_parameters)
...
#def wrapper(args,simulations):
#    return regret_vs_T(*args)

N = 50
result = run_parallel_simulations(wrapper,100,2,(N,),{})

#processes = 2
#simulations = 100
#N = 50
#p = mp.Pool(processes = processes)
#tasks = [(N,simulations/processes,"experiment2_{0}.log".format(i)) for i in xrange(processes)] # creating a number of jobs equal to the number of processes
#results = p.map_async(wrapper,tasks)
#p.close()
#p.join()
