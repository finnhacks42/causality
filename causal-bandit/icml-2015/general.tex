...
\todo{give reference}
The second observation is that in the parallel bandit setting the $m(\eta)$ given in this section coincides with the $m$ in Equation blah.
Recall in that setting that 
\eq{
\actions = \set{do()} \cup \set{do(X_i = j) : 1 \leq i \leq N \text{ and } j \in \set{0,1}}\,.
}
For $a = do()$ we let $\eta_a = 1/2$. Assume without loss of generality that $\P{X_i = 1} \geq \P{X_i = 0}$ for all $i$ and
$\P{X_i = 0}$ is monotone increasing in $i$. Then let $m = \max\set{i : \P{X_i = 0} \leq 1 / i}$, which is the same as in \S3. 
For $i \leq m$ and $a = do(X_i = 0)$ let $\eta_a = 1/(2m)$. Then it is easy to check that $m(\eta) \leq 2m$. 
