...
Define $Q = \sum_{a \in \calA} \eta_a \Ps_a$ be the mixture distribution.
...
\begin{proof}
First note that $|Z_a(X_t)| \leq B_a$ and that
\eq{
\Var_Q[Z_a(X_t)] 
&\leq \EE_Q[Z_a(X_t)^2] \\
...
\eq{
\EE_Q[Z] 
&= \EE_{\Ps_a}\left[Y \ind{R_a \leq B_a}\right] \\
&= \EE_{\Ps_a} Y - \EE_{\Ps_a} \left[Y\ind{R_a > B_a}\right] \\
&= \mu_a - \beta_a\,,
}
where $0 \leq \beta_a = \EE_{\Ps_a}[Y \ind{R_a > B_a}] \leq \Pn{a}{R_a > B_a}$ is the negative bias. 
The bias may be bounded in terms of $m(\eta)$ via an application of Markov's inequality.
\eq{
\beta_a \leq \Pn{a}{R_a > B_a} \leq \frac{\EE_{\Ps_a}[R_a(X)]}{B_a} \leq \frac{m(\eta)}{B_a}\,.
}
Let $\epsilon_a > 0$ be given by
\eq{
\epsilon = \sqrt{\frac{2m(\eta)}{T} \log\left(2T|\calA|\right)} + \frac{3B_a}{T} \log\left(2T|\calA|\right)\,.
}
Then by the union bound and Bernstein's inequality we have
\eq{
...
Algorithm \ref{alg:general} depends on a choice of sampling distribution $\eta$. In light of Theorem \ref{thm:general}
