...
$r : \{0,1\}^N \to \R$ is an arbitrary, fixed, and unknown function. 
...
\STATE Perform empty intervention $do()$; observe $\vec{X}_t$ and $Y_t$
\ENDFOR
...
\STATE Estimate probabilities: $\hat{p}_a = \frac{2 T_a}{T}$ and $\hat{V}_a = \frac{1}{\hat{p}_a}$
\ENDFOR
