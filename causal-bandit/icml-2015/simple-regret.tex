...
The fact that only link from each $X_1,\ldots,X_N$ to $Y$ is a direct, causal one means that $\P{Y|do(X_i=j)}=\P{Y|X_i=j}$. Thus we can create good estimators of the returns of those actions $do(X_i = j)$ for which $\P{X_i = j}$ is large. 
