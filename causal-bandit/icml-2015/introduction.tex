Medical drug testing, economic policy setting, and other scientific processes are commonly framed and analysed in the language of sequential experimental design and, in special cases, as bandit problems~\citep{Robbins1952,Chernoff1958}. 
In this framework, single actions or experiments from a pre-determined set are repeatedly performed in order to determine the most effective one via feedback from a single, real-valued reward signal. 
We consider the more general setting for sequential experiments where the feedback after each action consists of the reward signal as well as the values of other variables drawn from a probabilistic causal model~\citep{Pearl2000}.
By explicitly modeling this extra information (\eg, drug side-effects, secondary economic indicators) we show, theoretically and empircally, how it can be used to improve the rate at which effective actions can be identified.
...
This paper can be seen as a first step towards formalising and solving problems such as these. In \S\ref{sec:defs} we introduce \emph{causal bandit problems} in which interventions are treated as arms in a bandit problem but their inlfuence on the reward --- along with any other non-interventional observations --- is assumed to conform to a known causal graph. 
Although the general problem subsumes classical stochastic bandit problems as well as stochastic contextual bandits ... focus on parallel bandit problem.
