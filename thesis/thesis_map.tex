...
\item What are the nessessary and sufficient conditioins for decision making? How much about a the distribution of rewards for different arms must I know in order to select one (under different assumptions about the loss function). Ie when is the mean sufficient? What about the mean and the variance? - when can we take one set of results and compute something about a different loss function?
...
In chapter \ref{chap:interventionalist_view} I describe the interventionalist viewpoint on learning the outcome of actions. 

I review the key problems and results within the bandit liturature

highlight some interesting connections between existing strands of work on bandits and causal inference. 
...
The goal of causal inference is to learn the effect of taking an action. We can do this directly via experimental approaches, however any given agent only has a limited capacity to manipulate the world. We are generating and storing data on almost every aspect of our lives at an unprecedented rate. As we incorporate sensors and robotics into our cities, homes, cars, everyday products and even our bodies, the breadth and scale of this data will only increase. However, only a tiny fraction of this data will be generated in a controlled way with the specific goal of answering a single question. An agent that can only learn from data when it had explicit control (or perfect knowledge of) the process by which that data was generated will be severly limted. This makes it critical that we develop effective methods that us to predict the outcome of an intervention in some system from observing it. This is the problem of observational causal inference. 

\section{Causal models}
\label{sec:causal_models}

As previusly mentioned, observational causal inference is 

Observational causal inference aims to infer the outcome of an intervention in some system from data obtained by observing (but not intervening in) it. As previuosly mentioned, this is a form of transfer learning; we need to infer properties of the system post-intervention from observations of the system pre-intervention. Doing this requires some assumptions about how these two systems are related, or other words, a way of describing actions and how we anticipate the system will respond to them. Three key approaches have emerged: counterfactuals, structural equation models and causal Bayesian networks. 

counterfactuals 

MOTIVATE WHY I AM DESCRIBING THEM ALL SUMMARISE WHAT EACH OFFERS.


In this chapter we will examine the problems these approaches allow us to solve, the assumptions they rely on and how they differ. We will also use them to describe the following simplified examples. The aim is to demonstrate the notations and formalisms needed to tackle more interesting problems later on.
...
\end{tikzpicture}
\end{subfigure}
\caption{Some valid Bayesian networks for a distribution $P$ over $(X_1,X_2,X_3)$ in which $X_3$ is conditionally independent of $X_1$ given $X_2$, denoted $X_3 \ci X_1 | X_2$. Graphs (a), (b) and (c) are all a \emph{perfect map} for $P$ as the graphical stucture implies exactly the same set of independencies exhibitied by the distribution. Graph (d), like figure \ref{fig:bayesnet} does not imply any conditional independencies, and is thus a valid (but not very useful) Bayesian network representation for any distribution over three variables.}
\label{fig:bayesnet2}
\end{figure}
...
\subsubsection{Can't everything be encoded in a Bayesian model?}
...
\subsubsection{Estimating causal effects by adjusting for confounding variables}
...
\subsection{The counterfactual nature of regret}
\todo{Fix this bit up}

Note that we have used the conterfactual notation to represent the rewards given each action. The stochastic bandit problem can be defined without recourse to counterfactual variables by having the world stochastically generate only a reward for the selected action at each timestep. The psuedo-regret (equation \ref{eqn:pseudo_regret}) remains unchanged as both it and the learner's actions depend only on the marginal distributions over $\cfb{Y}{i}$ for each $i \in \set{1,...k}$. However, the regret as defined in equation \ref{eqn:raw_regret} is no longer properly defined as it relies on both $\cfb{Y}{a_t}$ and $\cfb{Y}{i^*}$ being defined at each timestep. The variance (but not expectation) of $\rawregret$ depends on the covariance between counterfactual variables.

 The learner is only able to observe properties of the marginal distributions over $\cfb{Y}{i}$ for each $i \in \set{1,...k}$.
 The stochastic bandit problem can be defined without recourse to counterfactual variables by  

 The algorithm and definition of regret depend only on the marginal distributions over $\cfb{Y}{i}$ for each $i \in \set{1,...k}$.
...
\todo{think carefully about the graphs in this sections}
...
\chapter*{Regret is a goat}

Consider a stochastic, two-armed bernoulli bandit with the (general) joint distribution of the (conterfactual) rewards $\P{\cf{Y}{1},\cf{Y}{2}}$ given in table \ref{table:joint_bernoulli_counterfactuals}. Suppose without loss of generality that arm 1 is the optimal arm.  

\begin{table}[h]
\caption{General joint distribution over two bernoulli random variables.}
\label{table:joint_bernoulli_counterfactuals}
\begin{center}
\begin{tabular}{ c | c | c }
$\cf{Y}{1}$ & $\cf{Y}{2}$ & $\P{\cf{Y}{1},\cf{Y}{2}}$ \\
  \hline			
  1 & 1 & $a$ \\
  1 & 0 & $p-a$ \\
  0 & 1 & $q-a$ \\
...
Recall that the regret is defined by,

\eqn{
\rawregret = \sum_{t=1}^T (\cfb{Y}{i^*} - \cfb{Y}{a_t})
}

Consider an algorithm that always selects arm 2. The variance (but not expectation) of the regret incured by this algorithm depends on the covariance between the counterfactual variables $\cf{Y}{1}$ and $\cf{Y}{2}$, which cannot estimated from data. 

\eqn{
\E{\rawregret} = \sum_{t=1}^T(\E{\cf{Y}{1}}-\E{\cf{Y}{2}}) = T(p-q)
}

\eqn{
\V{\rawregret} &= \E{\left(\sum_{t=1}^T (\cfb{Y}{1} - \cfb{Y}{2})\right)^2} - \left(\sum_{t=1}^T(\E{\cf{Y}{1}}-\E{\cf{Y}{2}})\right)^2\\
&= \sum_{t=1}^T \E{(\cfb{Y}{1} - \cfb{Y}{2})^2 - (\E{\cfb{Y}{1}}-\E{\cfb{Y}{2}})^2}\\
&= \sum_{t=1}^T\left(\V{\cfb{Y}{1}}+\V{\cfb{Y}{2}} - 2 Cov(\cfb{Y}{1},\cfb{Y}{2})\right)\\ &= T\left(p(1-p)+q(1-q) - 2(a-pq)\right)
}
...
The two cultures \citep{Breiman2001}
 - wrong because suggets we can just use the machine learning approach for everything. Does not address the problems that a) structural assumptions required for causal inference. b) we cannot use cross-vaidation to assess the validity of a causal model (really b is the whole point).
...
Definition as backwards. Explanatory models focus on theory because some theory (or equivelently stated assumptions) are required in order to predict the outcome of an intervention from observational data. 

Causal modelling frameworks allow us to seperate assumptions about the structure of the relationships between variables from assumptions about the functional form. 

What can be data driven and what cannot? (for a given set of assumptions). 


Allowing for the possibility that we might only need to be able to predict the outcome of an intervention on a given subset of variables (for example those over which we have the ability to manipulate) or the possibility that we can can obtain observation from directly intervening in a system creates creates a need for models that are neither explanatory or predictive in the languate of EoP but are built on a hybrid of both approaches. 
