...
\regret = \max_{h \in \mathcal{H}}\E{\sum_{t=1}^T{Y_{t,h(\vec{x}_t)}}} - \E{\sum_{t=1}^T{Y_{t,a_t}}}
}
...
Another topic of interest within the contextual bandit community that is deeply related to causal effect estimation from observational data is learning from logged bandit feedback data \citep{}. 

If no assumptions are made about the behaviour of the hypothosis that generated the data.... does it reduce to causal effect estimation?

However, in the applications considered in these setting such as ad serving, the details of the hypothosis generating the data are often known.  
