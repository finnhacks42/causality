...
Subtle modifications to the UCB algorithm can eliminate the logarithmic term equation \ref{eqn:ucb_worst_case_regret}. This yields regret $ \bigo{\sqrt{TK}}$ and closes the gap with the worst case lower bound \cite{Audibert2009,Lattimore2015}, whilst retaining a good problem dependent bound of the form achieved by UCB \cite{Lattimore2015}.
...
\todo{discuss the key ways in which the simple regret problem differs from cumulative regret}
...
If no assumptions are made about the behaviour of the hypothesis that generated the data.... does it reduce to causal effect estimation?

However, in the applications considered in these setting such as ad serving, the details of the hypothesis generating the data are often known.  
...
Relation to SUTVA assumption, selecting a action for one 'unit' does not effect the reward distributions for other units. 
...
\item When does the expected regret make sense as the thing we actually would wish to optimise? Is optimising the expected regret any different to optimising the pseudo-regret? Who is the definition of regret down to? Does this definition of regret come from learning with expert advise?
...
\item Bubeck 2012 states that if the adversary is oblivious, the pseudo-regret coincides with the standard regret, 'which is always the ultimate quantity of interest'. Why, they are not the same for stochastic bandits are they?
...
\item do continuous armed bandits implies that there are arms with any arbitrary gap in reward (including worst case gap for that T) and thus you always end up with something like worst case regret?)
