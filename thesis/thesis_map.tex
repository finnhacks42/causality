...
The problem of playing go is clearly causal from the interventionalist perspective. The agent wishes to learn the probability of a win given an action they take. However, 
...
... this is interesting maybe come back to it ... an alternate approach would be to try to forecast the probability of a win given each action given the current board state as context ... One approach to causal inference is indeed to learn about actions from taking actions (or observing the actions that other have taken). 

When can we learn from the actions others have taken? When there is no confounding. And does this hold with Go? Probably because the board state encapsulates everything that should determine what move is played. Learning directly from actions and trying to generalise (can in some instances reduce the problem to standard ML)
...
This is a trivial but significant observation. It comes down to asking, is the objective or loss function we are minimising a good proxy for our underlying goal in building the model. 
\begin{itemize}
...
How can we characterise which problems are predictive and which are causal?

Mention transferability viewpoint
...
Always, unless you can demonstrate otherwise.
