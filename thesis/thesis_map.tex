...
\section{What makes a causal inference problem?}
...
Having considered these examples we can now identify some general aspects of problems that require causal (as opposed to purely predicive) inference. A predictive model may be sufficient if;
...
\item 
This is more likely be be the case if the actions are recorded in the training data (unlike in the pneumonia example), and the algorithm has the same objective and information on which to base its decisions as generated the training data. 
...
We can also view causal inference problems as a particular form of the transfer learning problem. We have training data obtained from one system, but we wish te estimate 

It comes down to asking, is the objective or loss function we are minimising a good proxy for our underlying goal in building the model. 

Optimising hard to the wrong metric. 



In conclusion, it is rare that anyone would build a model without indending that some kind of decion be made on the basis of its results. For this reason we assume that a causal model is required unless we can justify otherwise. I hope these examples gave you a feel for the richness and subtitles of causal inference. We will return to some of them in more detail once we have established some more concrete language and tools to approach them with.
...
This stuff gets built into explaining how the thesis fits together.
