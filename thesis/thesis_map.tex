...
Note that we have used the counterfactual notation to represent the rewards given each action. The stochastic bandit problem can be defined without recourse to counterfactual variables by having the world stochastically generate only a reward for the selected action at each timestep. The pseudo-regret (equation \ref{eqn:pseudo_regret}) remains unchanged as both it and the learner's actions depend only on the marginal distributions over $\cfb{Y}{i}$ for each $i \in \set{1,...k}$. However, the regret as defined in equation \ref{eqn:raw_regret} is no longer properly defined as it relies on both $\cfb{Y}{a_t}$ and $\cfb{Y}{i^*}$ being defined at each timestep. The variance (but not expectation) of $\rawregret$ depends on the covariance between counterfactual variables.
...
\todo{write an intro to this chapter}
...
Consider a stochastic, two-armed Bernoulli bandit with the (general) joint distribution of the (counterfactual) rewards $\P{\cf{Y}{1},\cf{Y}{2}}$ given in table \ref{table:joint_bernoulli_counterfactuals}. Suppose without loss of generality that arm 1 is the optimal arm.  
...
Consider an algorithm that always selects arm 2. The variance (but not expectation) of the regret incurred by this algorithm depends on the covariance between the counterfactual variables $\cf{Y}{1}$ and $\cf{Y}{2}$, which cannot estimated from data. 
